...
std::vector<BethYw::InputFileSource> BethYw::parseDatasetsArg(
  ...
  int numInputs = static_cast<int>(inputDatasets.size());
  ...
  if (numInputs < 1){
	  ...
	  for (int i = 0; i < numInputs; i++){
	  	  if (inputDatasets[i] == "all") {
	  		  //If all keyword found will output all datasets
	  		  ...
	  		  //otherwise searches through to see if keyword is on the list
	  		  bool isDataset = false;
	  		  for (int x =0; x< numDatasetsInt; x++){
	  			  if (inputDatasets[i] == allDatasets[x].CODE){
	  				  isDataset = true;
